...
var Catalog = map[EventKind]EventMeta{
	...
	"personnel.profile_reminder": {Category: CatAccount, DefaultTier: TierImmediate, Severity: "warn", SubjectTpl: "Action needed — complete your profile by {{.Deadline}}", HeadlineTpl: "Complete your personnel profile by {{.Deadline}}", Description: "HR asked you to fill in your personnel profile (NIN, emergency contact, guarantor, CV…)", DefaultLink: "/my-work?tab=profile"},
	...
	"task.comment":             {Category: CatTasks, DefaultTier: TierImmediate,    Severity: "info",     SubjectTpl: "{{.Author}} commented on {{.Title}}",                 HeadlineTpl: "{{.Author}} commented on {{.Title}}",                 Description: "Someone commented on a task you created or are assigned to", DefaultLink: "/projects"},
